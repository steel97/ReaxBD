...
void main() {
  group('MemTable Tests', () {
    ...
    test('should get entries excluding tombstones', () {
      ...
      final keyStrings = entries.keys.map((k) => String.fromCharCodes(k)).toSet();
    ...
    test('should get all entries including tombstones', () {
      ...
      final hasKey2 = allEntries.keys.any((k) => 
          String.fromCharCodes(k) == String.fromCharCodes(key2));
      expect(hasKey2, isTrue);
      
      final key2Value = allEntries.entries.firstWhere((e) => 
          String.fromCharCodes(e.key) == String.fromCharCodes(key2)).value;
    ...
    test('should handle batch get operations', () {
      ...
      final keys = [[49], [50], [51], [52]]; // 52 doesn't exist
      final results = memtable.getBatch(keys);
      
      expect(results.length, equals(4));
      // Check results by finding matching keys
      final key49Result = results.entries.firstWhere((e) => 
          String.fromCharCodes(e.key) == '1').value;
      final key50Result = results.entries.firstWhere((e) => 
          String.fromCharCodes(e.key) == '2').value;
      final key51Result = results.entries.firstWhere((e) => 
          String.fromCharCodes(e.key) == '3').value;
      final key52Entry = results.entries.where((e) => 
          String.fromCharCodes(e.key) == '4').toList();
