...
void main() {
  group('Transaction Manager Tests', () {
    ...
    test('should begin and commit simple transaction', () async {
      ...
      await transaction.put('test_key', Uint8List.fromList('test_value'.codeUnits));
    ...
    test('should abort transaction on error', () async {
      ...
      // Put data in transaction
      await transaction.put('abort_key', Uint8List.fromList('abort_value'.codeUnits));
    ...
    test('should read own writes within transaction', () async {
      ...
      // Put data in transaction
      await transaction.put('row_key', Uint8List.fromList('row_value'.codeUnits));
    ...
    test('should execute transaction with automatic retry', () async {
      ...
      final result = await transactionManager.executeTransaction<String>(
        (transaction) async {
          ...
          await transaction.put('retry_key', Uint8List.fromList('retry_value'.codeUnits));
          return 'success';
        },
        maxRetries: 3,
      );
    ...
    test('should handle delete operations in transactions', () async {
      // Put initial data
      await storageEngine.put('delete_key'.codeUnits, Uint8List.fromList('delete_value'.codeUnits));
    ...
    test('should validate read set for repeatable read', () async {
      // Put initial data
      await storageEngine.put('validate_key'.codeUnits, Uint8List.fromList('initial'.codeUnits));
      ...
      await storageEngine.put('validate_key'.codeUnits, Uint8List.fromList('modified'.codeUnits));
    ...
    test('should handle multiple operations in transaction', () async {
      ...
      // Update existing
      await transaction.put('multi_1', Uint8List.fromList('updated_1'.codeUnits));
    ...
    test('should handle lock timeouts', () async {
      ...
      final lockFuture = txn2.put('lock_key', Uint8List.fromList('waiting'.codeUnits));
