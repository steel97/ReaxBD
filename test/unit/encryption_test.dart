...
import 'package:reaxdb_dart/src/core/encryption/encryption_type.dart';
import 'dart:io';
...
void main() {
  group('ReaxDB Encryption Tests', () {
    ...
    group('XOR Encryption', () {
      ...
      test('should provide encryption info for XOR encrypted database', () async {
        final encryptionInfo = db.getEncryptionInfo();
      ...
      test('should handle large data with XOR encryption efficiently', () async {
    ...
    group('AES-256 Encryption', () {
      ...
      test('should provide encryption info for AES-256 encrypted database', () async {
        final encryptionInfo = db.getEncryptionInfo();
      ...
      test('should compare performance across encryption types', () async {
        ...
        for (final configData in [
          ...
          final db = await ReaxDB.open(
            ...
            encryptionKey: configData['name'] != 'none' ? 'test_key_for_performance' : null,
          );
          ...
          try {
            ...
            stopwatch.stop();
            results[configData['name'] as String] = stopwatch.elapsedMilliseconds;
        ...
        expect(results['none']!, lessThan(results['aes256']!)); // No encryption should be fastest
        expect(results['xor']!, lessThan(results['aes256']!)); // XOR should be faster than AES-256
    ...
    group('Encryption Error Handling', () {
      test('should throw error when encryption key is missing for XOR', () async {
      ...
      test('should throw error when encryption key is missing for AES-256', () async {
    ...
    group('Encryption Type Information', () {
      test('should provide correct encryption type information', () {
        ...
        expect(EncryptionType.xor.securityLevel, equals('Low - Basic obfuscation only'));
        expect(EncryptionType.aes256.securityLevel, equals('High - Cryptographically secure'));
