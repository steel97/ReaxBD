...
class DatabaseService {
    ...
    for (final test in securityTests) {
      ...
      try {
        ...
        if (retrieved == test['payload']) {
          logs.add(
            '✅ ${test['name']}: Data stored/retrieved safely (${stopwatch.elapsedMicroseconds}μs)',
          );
        } else {
        ...
        stopwatch.stop();
        logs.add(
          '🛡️  ${test['name']}: Blocked by security (${e.toString().substring(0, 50)}...)',
        );
      }
    }

    logs.add(
      '🔒 Security test completed - ReaxDB safely handled all attack vectors',
    );
    return logs;
  }
  ...
  static Future<List<String>> runOptimizedConcurrencyTest() async {
    ...
    logs.add(
      '🔧 Testing with batch operations and performance optimizations...',
    );
  ...
  static Future<List<String>> runSecondaryIndexTest() async {
    ...
    if (userByEmail != null) {
      ...
      if (stopwatch1.elapsedMicroseconds > 0) {
        ...
        logs.add(
          '   🚀 Speedup: ${speedup.toStringAsFixed(1)}x faster with index!',
        );
      }
    }
    ...
    logs.add(
      '   Average query time with index: ${avgQueryTime.toStringAsFixed(1)}μs',
    );
    logs.add(
